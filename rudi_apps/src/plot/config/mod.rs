...
/// 
/// If `x_range` or `y_range` are None, axis ranges will be calculated from the 
/// data. Values are provided as `Range<T>`, e.g., `0..10`.
/// 
...
/// 
/// `plot_area_inches` defaults to 1.75 inches wide x 1.25 inches high, NOT 
...
/// 
/// `print_dpi` defaults to 600 and rarely needs to be changes. All plots are 
/// rendered at `canvas_size` and `print_dpi` and downsized for the screen to 
/// ensure WYSIWYG image downloads.
      ...
      Y: 'static + Clone + PartialEq + PartialOrd
{
    ...
    pub x_range: Option<Range<X>>,
    pub y_range: Option<Range<Y>>,
      ...
      Vec<Option<Y>>: ColVec,
{
    ...
    ///     - `y_range()`
    ///     - `plot_area_inches()` (default is 1.75 x 1.25 inches)
    ...
    pub fn builder() -> Self {
        Self {
            ...
            y_label: None,
            title:   None,
            plot_area_inches: PrintArea { width: 1.75, height: 1.25 },
            print_dpi:  600,
        }
    }
    ...
    //     self
    // }
    /// Set the title (default is None).
    pub fn title(mut self, title: &str) -> Self {
        self.title = Some(title.to_string());
        self
    }
    /// Set the X-axis label (default is None).
    pub fn x_label(mut self, label: &str) -> Self {
        self.x_label = Some(label.to_string());
        self
    }
    /// Set the Y-axis label (default is None).
    pub fn y_label(mut self, label: &str) -> Self {
        self.y_label = Some(label.to_string());
        self
    }
    /// Set the X-axis range (default determines the range from the data).
    pub fn x_range(mut self, range: Range<X>) -> Self {
        self.x_range = Some(range);
        self
    }
    /// Set the Y-axis range (default determines the range from the data).
    pub fn y_range(mut self, range: Range<Y>) -> Self {
        self.y_range = Some(range);
        self
    }
    /// Set the chart plotting area size in inches, NOT including the axis,  
    /// title, and legend labeling areas (default is 1.75 width x 1.25 height).
    pub fn plot_area_inches(mut self, width: f32, height: f32) -> Self {
        self.plot_area_inches = PrintArea { width, height };
        self
    }
    /// Set the print DPI for downloaded images (default is 600).
...
impl PlotGridConfig {
    ...
    where X: 'static + Clone + Copy + PartialEq + PartialOrd + Debug,
        Y: 'static + Clone + Copy + PartialEq + PartialOrd + Debug,
        Range<X>: AsRangedCoord<Value = X>,
        Range<Y>: AsRangedCoord<Value = Y>,
        <Range<X> as AsRangedCoord>::CoordDescType: ValueFormatter<X>,
        <Range<Y> as AsRangedCoord>::CoordDescType: ValueFormatter<Y>,
