...
/// 
/// An `InputPanel` expects one or more `InputRow` as children, each with one 
/// or more RuDI input elements. 
/// 
...
/// 
/// `default_input_width` is the default pixel width for all inputs in the 
/// panel, which defaults to 75px. Each input element width can be overridden 
/// using its `width` property.
/// 
...
/// One row of user inputs to be placed into an `InputPanel`
/// and populated with one or more RuDI input elements. 
#[component]
