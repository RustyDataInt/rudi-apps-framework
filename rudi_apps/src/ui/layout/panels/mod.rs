...
/// Get a `Signal<DataFrame>` for a `PlotPanel` or `TablePanel` from its `data` 
/// and `data_frame` properties. If `data_frame` is set it is used as is, 
/// otherwise `data` must be provided in which case the `Vec<T>` is converted 
/// to a `DataFrame` for internal use.
fn get_df<T> (
